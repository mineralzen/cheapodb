...
if __name__ == '__main__':
    import dask
    db = Database(name='uswaterdb',
                  auto_create=False,
                  iam_role_arn='arn:aws:iam::193000466339:role/service-role/uswaterdb-CheapoDBExecutionRole'
                  )
    table = Table(name='demo', db=db, prefix='demo')
    db.create_crawler('demo', table_prefix='demo')
    dtype = {'DILandingZone': 'object',
             'DIPlay': 'object',
             'EndDateFirstTreatmentJob': 'object',
             'FracVendor': 'object',
             'LASMnemonics': 'object',
             'PlayAssessmentBasin': 'object',
             'ProducingEntityID': 'object',
             'ProducingEntityLeaseNumber': 'object',
             'QualifierFirstTreatmentJob': 'object',
             'QuarterQuarter': 'object',
             'Range': 'object',
             'StartDateFirstTreatmentJob': 'object',
             'ToeInLandingZone': 'object',
             'Township': 'object',
             'TreatmentType': 'object'}
    dtype.update({'Abstract': 'object',
       'Block': 'object',
       'DISubplay': 'object',
       'District': 'object',
       'FirstTestChokeSize': 'object',
       'Section': 'object',
       'Survey': 'object'})
    df = dask.dataframe.read_csv(
        '/home/cole/Workspace/cheapodb/well-rollups.csv',
        dtype=dtype
    )
    table.from_dataframe(df)
    db.update_tables('demo')
